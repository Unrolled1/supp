// ============================================
// فایل مدیریت فعالیت‌ها
// ============================================
function openEditModal(id, name) {
    if (!name) {
        const row = document.querySelector(`#activity_${id}`);
        if (row) {
...
function saveEdit() {
    const activity_id = document.getElementById('edit_activity_id').value;
    const name = document.getElementById('edit_name').value;

    if (!name.trim()) {
        Swal.fire({
            title: 'خطا!',
            text: 'نام فعالیت الزامی است',
            icon: 'warning',
    ...
    formData.append('edit_activity', '1');
    formData.append('activity_id', activity_id);
    formData.append('name', name);

    fetch('admin_activities.php', {
        method: 'POST',
        body: formData
    })
        ...
        .then(data => {
            if (data.success) {
                ...
                const row = document.querySelector(`#activity_${data.id}`);
                if (row) {
                    const nameCell = row.querySelector('td:nth-child(2)');
                    if (nameCell) {
                ...
                Swal.fire({
                    title: 'موفق!',
                    text: 'فعالیت با موفقیت ویرایش شد.',
                    icon: 'success',
                    timer: 1500,
